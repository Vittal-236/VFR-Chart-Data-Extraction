...
DOT_BELOW_FRAC        = 0.0
DOT_SEARCH_H_FRAC     = 0.25
DOT_SEARCH_W_FRAC     = 0.18
DOT_LEFT_CENTRE_FRAC  = 0.15   # left dot at 15% of template width
DOT_RIGHT_CENTRE_FRAC = 0.85   # right dot at 85% of template width
DOT_MIN_AREA          = 3
DOT_MAX_AREA          = 35
DOT_MAX_ECCENTRICITY  = 0.75
...
def verify_two_dots(dot_binary: np.ndarray, candidate: dict) -> dict:
    """
    Search for two dots below the double obstacle bounding box.

    Left dot  centred at 15% of template width.
    Right dot centred at 85% of template width.
    Soft gate only — detection kept even if dots not found.
    """
    r0, c0 = candidate["row"], candidate["col"]
    tw, th = candidate["scale_w"], candidate["scale_h"]
